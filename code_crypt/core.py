...
class CodeCrypt:
    ...
    def __init__(
        ...
        self.public_key_file = os.path.join(self.environment_keys_dir, "public_key.asc")
    ...
    def _init_dirs(self, data_dir):
        ...
        self.environment_keys_dir = os.path.join(data_dir, "keys", self.app_environment)
    ...
    def _get_plaintext_private_key(self, ciphertext_blob, encryption_context={}):
        ...
        try:
            ...
            raise errors.KmsError(
                f"Could not decrypt private key using KMS key '{self.kms_key_id}' (Reason: {str(e)})"
            )
    ...
    def _set_encryptor(self, public_key):
        ...
        if not public_key:
            try:
                ...
                raise errors.InputError(
                    f"public key '{self.public_key_file}' does not exist."
                )
        ...
        try:
            ...
            raise errors.EncryptorError(f"public key is malformed. (Reason: {str(e)})")

    def _set_decryptor(self, plaintext_private_key=None, encrypted_private_key=None):
        """Creates a OAEP decryptor based on a RSA private key."""
        ...
        if not plaintext_private_key:
            if not encrypted_private_key:
                try:
                    ...
                    raise errors.InputError(
                        f"private key '{self.encrypted_private_key_file}' does not exist."
                    )

            plaintext_private_key = self._get_plaintext_private_key(
                encrypted_private_key, encryption_context=self.encryption_context
            )

        try:
            private_key_obj = serialization.load_pem_private_key(
                plaintext_private_key, password=None, backend=default_backend()
            )

            self.decryptor = Decryptor(private_key_obj)
        except Exception as e:
            raise errors.DecryptorError(f"private key is malformed. (Reason: {str(e)})")
    ...
    def _validate_secret(self, secret_name, secret, blob_mode):
        if not blob_mode:
            ...
            if len(secret_name) == 0:
                raise errors.InputError(
                    f"secret name '{secret_name}' must be greather than length of 0"
                )

            # secret name can only contain ascii characters
            if not self._is_ascii(secret_name):
                raise errors.InputError(
                    f"secret name '{secret_name}' must contain only ASCII chararacters"
                )

        # warn when secret value contains non-ascii chars
        if not self._is_ascii(secret):
            log.warning("Secret '%s' contains non-ASCII characters!", secret_name)

        # warn when secret value contains escape characters (e.g. "\r")
        if "\\" in secret:
            log.warning("Secret '%s' contains escape sequence characters!", secret_name)
    ...
    def generate_key_pair(self):
        ...
        private_key_file_exists = os.path.exists(self.encrypted_private_key_file)
        ...
        if public_key_file_exists and private_key_file_exists:
            ...
            raise errors.CodeCryptError("public key exists but private key is missing.")
        elif private_key_file_exists and not public_key_file_exists:
            raise errors.CodeCryptError("private key exists but public key is missing.")
        ...
        try:
            ...
            raise errors.KmsError(
                f"Could not encrypt private key using KMS key '{self.kms_key_id}' (Reason: {str(e)})"
            )
    ...
    def decrypt(
        self, secret_name=None, plaintext_private_key=None, encrypted_private_key=None
    ):
    ...
    def blob_decrypt(
        self, secret_blob, plaintext_private_key=None, encrypted_private_key=None
    ):
